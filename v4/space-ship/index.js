...
var ENEMY_FREQ = 1500;
var Enemies = Rx.Observable.interval(ENEMY_FREQ).scan(function(enemyArray) {
  var enemy = {
    x: parseInt(Math.random() * canvas.width),
    y: -30
  };
...
var playerFiring = Rx.Observable.merge(
  ...
  Rx.Observable.fromEvent(canvas, 'keyup').filter(function(e) {
    return e.keyCode === 32;
  })
)
  .sample(200)
...
var HeroShots = Rx.Observable.combineLatest(playerFiring, SpaceShip, function(shotEvents, spaceShip) {
  return {
...
var Enemies = Rx.Observable.interval(ENEMY_FREQ).scan(function(enemyArray) {
  var enemy = { x: parseInt(Math.random() * canvas.width), y: -30, shots: [] };
...
var Game = Rx.Observable.combineLatest(starObservable, SpaceShip, Enemies, HeroShots, function(
  stars,
  spaceship,
  enemies,
  heroShots,
